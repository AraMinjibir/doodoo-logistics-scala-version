error id: file://<WORKSPACE>/app/domain/models/Shipment.scala:scala/package.IllegalArgumentException#
file://<WORKSPACE>/app/domain/models/Shipment.scala
...
offset: 2843
uri: file://<WORKSPACE>/app/domain/models/Shipment.scala
text:
```scala
...
def create(lengthInCentimeters: Double, widthInCentimeters: Double, heightInCentimeters: Double): Either[List[Throwable], Dimensions] = {
  ...
  def createDimension(lengthInCentimeters: Double, widthInCentimeters: Double, heightInCentimeters: Double): Either[List[Throwable], Dimensions] = {
    // val a = Option.when(lengthInCentimeters <= 0)
    
    // val b: Either[IllegalArgumentException, Double] = Either.cond(
    //   lengthInCentimeters <= 0,
    //   lengthInCentimeters,
    //   new IllegalArgumentException(s"Package length must not be < 0: $lengthInCentimeters")
    // )

    // val errors = List(
    //   if(lengthInCentimeters <= 0) new IllegalArgumentException(s"Package length must not be < 0: $lengthInCentimeters"),
    //   if (widthInCentimeters <= 0) new IllegalArgumentException(s"Width length must not be < 0: $widthInCentimeters"),
    //   if (heightInCentimeters <= 0) new IllegalArgumentException(s"Height length must not be < 0: $heightInCentimeters")
    // )

    val err: List[IllegalArgumentException] = List(
      Option.when(lengthInCentimeters <= 0)(new IllegalArgumentException(s"Package length must not be < 0: $lengthInCentimeters")),
      Option.when(widthInCentimeters <= 0)(IllegalArgumentException(s"Width length must not be < 0: $widthInCentimeters")),
      Option.when(heightInCentimeters <= 0)(new IllegalArgumentException(s"Height length must not be < 0: $heightInCentimeters")),
    ).flatten

    Either.cond(
      lengthInCentimeters > 0 && widthInCentimeters > 0 && heightInCentimeters > 0,
