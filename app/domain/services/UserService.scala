...
trait UserService {
  ...
  def createUser(dto:UsersCreationDto): Future[Either[String, User]]
  ...
  def updateUser(userId:UUID, updateData: UsersUpdateDto): Future[Either[String, User]]
  def deleteUser(userId:UUID):Future[Either[String, User]]
